...
const Navbar = () => {
  ...
  const navLinks = [{ name: "New Arrivals", href: "#home" }, { name: "Catalog", href: "#shop" }, { name: "Categories", href: "#shop" }, { name: "About", href: "#about" }];
  ...
  return (
    <nav className="fixed w-full z-[100] bg-white/80 backdrop-blur-xl border-b border-slate-100 py-3 md:py-4 px-4 md:px-12">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <a href="#home" onClick={(e) => { e.preventDefault(); handleLinkClick("#home"); }} className="text-xl md:text-2xl font-black tracking-tighter text-slate-900">ALPHA<span className="text-[#10B981]">OMEGA</span></a>
        ...
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="absolute top-0 left-0 w-full min-h-screen bg-white z-[105] flex flex-col items-center justify-center lg:hidden" >
            <div className="flex flex-col items-center space-y-8 text-center">
               ...
               <a href="#contact" onClick={(e) => { e.preventDefault(); handleLinkClick("#shop"); }} className="bg-[#10B981] text-white font-black py-4 px-12 rounded-2xl text-sm uppercase tracking-widest shadow-xl" > Inquiry Now </a>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-[1400px] w-full grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
        <div className="lg:col-span-4 z-20 order-2 lg:order-1">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} >
            <div className="flex items-center space-x-3 mb-8"> <span className="w-10 h-[1px] bg-[#10B981]"></span> <span className="text-[10px] font-black text-[#10B981] uppercase tracking-[0.4em]">Curated Luxury</span> </div>
            <h1 className="text-4xl md:text-8xl font-black text-slate-900 leading-[1.1] md:leading-[0.9] tracking-tighter mb-8 md:mb-10 uppercase"> UPGRADE <br /> <span className="text-[#10B981] italic">YOUR LIFE.</span> </h1>
            <div className="space-y-6 md:space-y-8 mb-10 md:mb-12">
               <div className="flex items-start space-x-4"> <div className="bg-white p-3 rounded-2xl shadow-sm text-[#10B981] flex-shrink-0"><Tag size={18} md:size={20} /></div> <div> <h4 className="font-black text-slate-900 text-xs md:text-sm uppercase">Exclusive Access</h4> <p className="text-slate-400 text-[10px] md:text-xs">Handpicked premium items you won't find elsewhere.</p> </div> </div>
               <div className="flex items-start space-x-4"> <div className="bg-white p-3 rounded-2xl shadow-sm text-[#10B981] flex-shrink-0"><Zap size={18} md:size={20} /></div> <div> <h4 className="font-black text-slate-900 text-xs md:text-sm uppercase">Instant Support</h4> <p className="text-slate-400 text-[10px] md:text-xs">Order directly via WhatsApp for lightning fast response.</p> </div> </div>
            </div>
            <a href="#shop" className="inline-flex items-center space-x-4 md:space-x-6 group"> <div className="bg-slate-900 text-white w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all group-hover:bg-[#10B981] group-hover:scale-110"> <ArrowRight size={24} /> </div> <span className="text-xs md:text-sm font-black text-slate-900 uppercase tracking-widest border-b-2 border-[#10B981] pb-1">Shop Collection</span> </a>
          </motion.div>
        </div>
        <div className="lg:col-span-8 relative h-[350px] sm:h-[500px] md:h-[800px] flex items-center order-1 lg:order-2">
           <AnimatePresence mode="wait">
             <motion.div key={activeBanner} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.8 }} className="w-full h-full rounded-[40px] md:rounded-[80px] overflow-hidden relative shadow-2xl border-4 border-white" >
                ...
                <div className="absolute bottom-6 left-6 right-6 md:bottom-12 md:left-12 md:right-12 flex flex-col md:flex-row justify-between items-end gap-4 md:gap-6">
                   <div className="bg-white/10 backdrop-blur-xl p-6 md:p-8 rounded-2xl md:rounded-3xl border border-white/20 w-full md:w-auto">
                      ...
                      <div className="flex items-center space-x-4"> <span className="text-xl md:text-2xl font-bold text-white">{banners[activeBanner].price}</span> <span className="text-[#10B981] font-black text-[8px] md:text-[10px] uppercase tracking-widest flex items-center gap-1"><Eye size={12} md:size={14} /> View Details</span> </div>
                   </div>
    ...
    <motion.div whileHover={{ y: -10 }} className="bg-white rounded-[30px] md:rounded-[40px] overflow-hidden product-shadow transition-all group" >
      <div className="aspect-square overflow-hidden relative">
        ...
        <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4">
           <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="bg-slate-900 text-white font-black py-3 md:py-4 px-6 md:px-8 rounded-xl md:rounded-2xl flex items-center transform translate-y-4 group-hover:translate-y-0 transition-transform shadow-2xl text-xs md:text-base" > WHATSAPP <MessageCircle className="ml-2 md:ml-3 text-[#10B981]" size={16} md:size={18} /> </a>
        </div>
      </div>
      <div className="p-6 md:p-8">
        <div className="flex justify-between items-start mb-2 md:mb-4"> <h4 className="text-lg md:text-xl font-black text-slate-900 tracking-tight">{product.name}</h4> <span className="text-lg md:text-xl font-bold text-[#10B981]">${product.price}</span> </div>
        <div className="flex items-center space-x-1 mb-4 md:mb-6 text-yellow-400"> {[...Array(5)].map((_, i) => <Star key={i} size={12} fill="currentColor" />)} <span className="text-slate-400 text-[8px] md:text-[10px] ml-2 font-black uppercase tracking-widest">Verified Seller</span> </div>
  ...
  <section id="shop" className="py-20 md:py-32 px-4 md:px-12 bg-white">
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-20 gap-8">
        <div> <h2 className="text-[#10B981] text-[10px] font-black uppercase tracking-[0.5em] mb-4">The Collection</h2> <h3 className="text-4xl md:text-7xl font-black text-slate-900 tracking-tighter uppercase leading-[0.9]">ESSENTIALS <br /> REFINED.</h3> </div>
        <button className="flex items-center space-x-3 px-6 py-3 md:px-8 md:py-4 bg-slate-50 rounded-xl md:rounded-2xl text-[10px] font-black text-slate-500 hover:text-[#10B981] transition-all uppercase tracking-widest w-full md:w-auto justify-center"> <Filter size={18} /> <span>Filter Items</span> </button>
      </div>
      ...
      <div className="mt-12 md:mt-20 text-center"> <button className="bg-slate-900 text-white font-black py-5 px-12 md:py-6 md:px-16 rounded-2xl md:rounded-[30px] text-[10px] uppercase tracking-[0.3em] hover:bg-[#10B981] transition-all shadow-2xl w-full md:w-auto"> Explore All Items </button> </div>
    </div>
  </section>
);
    ...
    <section id="about" className="py-20 md:py-32 px-4 md:px-12 bg-slate-50 border-y border-slate-100">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
        <div className="flex flex-col items-center text-center group">
          <div className="w-20 h-20 md:w-24 md:h-24 rounded-[30px] md:rounded-[40px] bg-white flex items-center justify-center text-[#10B981] mb-6 md:mb-8 shadow-xl transition-transform group-hover:scale-110"> <MessageCircle size={32} /> </div>
          <h4 className="text-xl md:text-2xl font-black text-slate-900 mb-3 md:mb-4 uppercase tracking-tight italic">Direct Order</h4>
          <p className="text-slate-500 font-light leading-relaxed text-sm max-w-[250px]">No complicated checkout. Chat directly with us on WhatsApp.</p>
        </div>
        <div className="flex flex-col items-center text-center group">
          <div className="w-20 h-20 md:w-24 md:h-24 rounded-[30px] md:rounded-[40px] bg-white flex items-center justify-center text-[#10B981] mb-6 md:mb-8 shadow-xl transition-transform group-hover:scale-110"> <Truck size={32} /> </div>
          <h4 className="text-xl md:text-2xl font-black text-slate-900 mb-3 md:mb-4 uppercase tracking-tight italic">Fast Delivery</h4>
          <p className="text-slate-500 font-light leading-relaxed text-sm max-w-[250px]">Fast and reliable shipping across Port-au-Prince and beyond.</p>
        </div>
        <div className="flex flex-col items-center text-center group">
          <div className="w-20 h-20 md:w-24 md:h-24 rounded-[30px] md:rounded-[40px] bg-white flex items-center justify-center text-[#10B981] mb-6 md:mb-8 shadow-xl transition-transform group-hover:scale-110"> <ShieldCheck size={32} /> </div>
          <h4 className="text-xl md:text-2xl font-black text-slate-900 mb-3 md:mb-4 uppercase tracking-tight italic">Elite Quality</h4>
          <p className="text-slate-500 font-light leading-relaxed text-sm max-w-[250px]">We guarantee the authenticity and quality of every single item.</p>
        </div>
      </div>
    </section>
  );
    ...
    <div className="max-w-7xl mx-auto relative z-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-16 mb-20 md:mb-32">
        <div className="lg:col-span-1">
          <div className="text-2xl md:text-3xl font-black tracking-tighter mb-8 md:mb-10">ALPHA<span className="text-[#10B981]">OMEGA</span></div>
          <p className="text-slate-400 font-light leading-relaxed mb-10 max-w-xs text-sm"> Defining the new standard for premium local shopping. Quality first, always. </p>
          <div className="flex space-x-6 md:space-x-8 text-slate-500"> <Instagram className="hover:text-[#10B981] transition-colors cursor-pointer" size={20} md:size={24} /> <Twitter className="hover:text-[#10B981] transition-colors cursor-pointer" size={20} md:size={24} /> <Facebook className="hover:text-[#10B981] transition-colors cursor-pointer" size={20} md:size={24} /> </div>
        </div>
        <div> <h5 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] mb-8 md:mb-12 text-slate-500">Collection</h5> <ul className="space-y-4 md:space-y-6 text-white text-xs md:text-sm font-bold uppercase tracking-widest"> <li><a href="#home">New Arrivals</a></li> <li><a href="#shop">Best Sellers</a></li> <li><a href="#shop">Limited Edition</a></li> </ul> </div>
        <div className="hidden sm:block"> <h5 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] mb-8 md:mb-12 text-slate-500">Service</h5> <ul className="space-y-4 md:space-y-6 text-white text-xs md:text-sm font-bold uppercase tracking-widest"> <li><a href="#" className="hover:text-[#10B981] transition-all">WhatsApp Concierge</a></li> <li><a href="#" className="hover:text-[#10B981] transition-all">Shipping Policy</a></li> <li><a href="#" className="hover:text-[#10B981] transition-all">FAQ</a></li> </ul> </div>
        <div> <h5 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] mb-8 md:mb-12 text-slate-500">Newsletter</h5> <div className="flex flex-col space-y-4 md:space-y-6"> <input type="email" placeholder="Email Address" className="bg-white/5 border border-white/10 p-5 rounded-xl md:rounded-2xl outline-none focus:border-[#10B981] transition-all text-xs md:text-sm font-light" /> <button className="bg-[#10B981] text-white font-black py-4 md:py-5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] uppercase tracking-[0.3em]">Join the Elite</button> </div> </div>
      </div>
      <div className="text-[8px] md:text-[9px] text-slate-600 font-black uppercase tracking-[0.4em] md:tracking-[0.5em] pt-8 md:pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6"> <p className="text-center md:text-left">© 2026 ALPHAOMEGA EXCLUSIVE — HAITI</p> <p className="italic">Architected with Excellence by Daky_400$/day</p> </div>
    </div>
  </footer>
);
